create table ClientMaster(CLIENTNO VARCHAR(6) PRIMARY KEY,NAME VARCHAR(20) NOT NULL,ADDRESS1 VARCHAR(30),
...
400054,'Maharashtra',15000);
 INSERT INTO ClientMaster VALUES('C00002','ANJU','B/11','CBT','PATTAMBI',679337,'KERALA',30000);
...
foreign key (CLIENTNO)REFERENCES ClientMaster(CLIENTNO),foreign key (SALESMANNO)REFERENCES SALESMAN_MASTER(SALESMANNO));
...
SELECT * FROM ClientMaster WHERE CLIENTNO = 'C00001' OR CLIENTNO = 'C00002';
UPDATE Product_Master SET SELLPRICE='1150.50'where SELLPRICE='350';
DELETE FROM ClientMaster WHERE CLIENTNO='C00002';
SELECT * FROM ClientMaster WHERE CITY LIKE '_U%';
SELECT COUNT(*) AS NUMBER FROM Product_Master  WHERE SELLPRICE>=1500;

--commands
ALTER TABLE ClientMaster ADD Phone_no NUMERIC;
...
select Product_Master.PRODUCTNO from Product_Master
JOIN ClientMaster ON ClientMaster.CLIENTNO=SALES_ORDER.CLIENTNO
...
WHERE ClientMaster.NAME='Ivan Bayross';
